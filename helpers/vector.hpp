...
#define ALIGN_ON_POW2(x, pow2) ( ( (x) + ((pow2)-1) ) & ~((pow2) - 1) )
	...
	struct dynarray8 {
		...
		static dynarray8 create(uint64_t const n_octets)
		{
			...
			uint64_t const mem_size =
				ALIGN_ON_POW2(mem_size, 4096);
			uint64_t const begin =
				(uint64_t) (malloc_aligned_longterm(4096, mem_size));
			if (begin)
		...
		{
			malloc_free_longterm(arr->begin);
	...
	struct dynarray {
		...
		{
			*this = dynarray<T>::create_with_size(4096);
		}
		...
		{
			*this = dynarray<T>::create_with_size(size);
		}
